...
export default function FloatingFilter() {
  ...
  return (
    <div
      className="fixed right-6 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col items-stretch"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <div
        className={`flex flex-col items-stretch bg-surface/70 backdrop-blur-xl border border-outline-variant/40 shadow-[0_8px_32px_rgba(0,0,0,0.15)] transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden ${
          hovered ? "rounded-[28px] w-[180px] py-5 px-3" : "rounded-full w-[72px] h-[72px] justify-center items-center"
        }`}
      >
        {!hovered ? (
          <button
